...
from n2v.nets.seattention import SEAttention
'''import pytorch
...
def unet_block(n_depth=2, n_filter_base=16, kernel_size=(3,3), n_conv_per_depth=2,
               ...
               skip_skipone=False):
    ##seattation模块
    se_module = SEAttention(channel=512, reduction=8)
    ...
    def _func(input):
        ...
        #改变卷积核
        kernel_sizes = (5,5,5)
        layer = conv_block(n_filter_base * 2 ** max(0, n_depth - 1), *kernel_sizes,
                           dropout=dropout,
                           activation=activation,
                           init=kernel_init,
